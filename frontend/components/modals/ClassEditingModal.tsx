...
interface ClassEditingModalProps {
  isOpen: boolean
  onClose: () => void
  classData: any
...
export function ClassEditingModal({ 
    ...
    if (isOpen) {
      ...
      if (classData) {
        ...
        setFormData({
          ...
          agenda: classData.agenda || '',
          scheduledStartTime: formattedStartTime,
          scheduledEndTime: formattedEndTime,
          instructorId: classData.instructorId || classData.instructor?._id || '',
          settings: {
            maxParticipants: classData.settings?.maxParticipants || 100,
            allowRecording: classData.settings?.allowRecording ?? true,
            allowScreenShare: classData.settings?.allowScreenShare ?? true,
            allowWhiteboard: classData.settings?.allowWhiteboard ?? true,
            allowChat: classData.settings?.allowChat ?? true,
            allowStudentMic: classData.settings?.allowStudentMic ?? false,
            allowStudentCamera: classData.settings?.allowStudentCamera ?? false,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const updatedClass = await api.updateLiveClass(classData.id || classData._id, {
        ...formData,
        updatedAt: new Date().toISOString()
      })
      onClassUpdated(updatedClass.data)
  ...
  return (
    <Modal 
      isOpen={isOpen} 
      onClose={onClose} 
      title={`Edit Class: ${classData?.title || 'Untitled'}`}
      size="xl"
    >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              name="title"
              required
              className="w-full p-2.5 border rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-opacity-50"
              style={{ 
        ...
        <div className="space-y-4 pt-4 border-t" style={{ borderColor: 'var(--color-border)' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {settingsList.map(setting => {
              ...
              return (
                <div key={setting.key} className="flex items-center gap-2 p-2 rounded-md transition-colors hover:bg-surface-hover">
                  <input
                    type="checkbox"
                    id={`edit-${setting.key}`}
                  ...
                  <label htmlFor={`edit-${setting.key}`} className="text-sm" style={{ color: 'var(--color-text)' }}>
                    {setting.label}
                  </label>
                </div>
              )
          ...
          {formData.materials.length === 0 ? (
            ...
            <div className="space-y-3">
              {formData.materials.map((material, index) => (
                <div key={index} className="grid grid-cols-12 gap-2 items-center p-3 rounded-md" style={{ backgroundColor: 'var(--color-surface-muted)' }}>
